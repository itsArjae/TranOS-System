...
export default function AdminTables(props) {
  ...
  const DisplayItems = rawGoodsData
    .filter((val) => {
      if (searchTerm == "") {
        return val;
      } else if (
        val.rawGoodsName.toLowerCase().includes(searchTerm.toLowerCase())
      ) {
        return val;
    ...
    .map((data) => {
      return (
        <div className={styles.Table__Data} key={data.id}>
          ...
          <div className={styles.Table__Data__Box}> {data.rawGoodsName}</div>
          <div className={styles.Table__Data__Box}>
            {data.Details + data.Unit}
          </div>
          <div className={styles.Table__Data__Box}>{data.Date}</div>
        </div>
      );
    });
  const pageCount = Math.ceil(rawGoodsData.length / itemsPerPage);
        ...
        <div className={styles.Table__Search_Form}>
          ...
          >
            <input
              ...
              placeholder="Search Raw Goods Name"
              onChange={(event) => {
